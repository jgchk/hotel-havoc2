...
import com.jgchk.hotelhavoc.network.CookReading
import javax.inject.Inject
import kotlin.concurrent.fixedRateTimer
import kotlin.math.abs

class Cook
@Inject constructor(private val cookReading: CookReading, onCompleteListener: OnCompleteListener) :
    Action(onCompleteListener) {

    override val name = "cook"
    override val drawable = R.drawable.ic_pan
    ...
    companion object {
        ...
        private val TAG = Cook::class.simpleName
    }
    ...
    override fun start() {
        ...
        fixedRateTimer(name, false, 0L, INCREMENT) {
            ...
            if (run) {
                cookReading.invoke(UseCase.None()) { it.either(::handleFailure, ::handleCookReading) }
            } else {
                cancel()
            }
        }
    ...
    private fun handleCookReading(reading: AccelerationReading) {
